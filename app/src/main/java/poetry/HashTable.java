...
public class HashTable<K, V> {
     ...
     * Implementation issue: This routine doesn't allow you to use a lazily deleted
     * location. Do you see why?
     *
    ...
    public boolean insert(K key, V val) {
        ...
        storage[currentPos] = new HashEntry<>(key, val, true);
    ...
    public String toString(int limit) {
        ...
        for (int i = 0; i < storage.length && count < limit; i++) {
            if (storage[i] != null && storage[i].isActive) {
                sb.append(String.format("%d: %s[%s]\n", i, storage[i].key.toString(), storage[i].val));
                count++;
            }
        }
        return sb.toString();
    ...
    private class HashEntry<K, V> {
        ...
        public V val; // the value
        public boolean isActive; // false if marked deleted

        public HashEntry(K key, V val, boolean active) {
            this.key = key;
            this.val = val;
